module imem #(
  ...
  always_ff @(posedge wb_clk_i or posedge wb_rst_i) begin : imem_ff
    if (wb_rst_i) begin
      ...
      if (wbs_cyc_i && wbs_stb_i) begin
        if (wbs_we_i) begin
          logic [31:0] address;
          logic [31:0] sram_mem[7:0];  // Pointer to selected SRAM

          // Select memory and address
          ...
          // Write operation
          if (address < 8) begin
            if (wbs_sel_i[0]) sram_mem[address][7:0] <= wbs_dat_i[7:0];
            if (wbs_sel_i[1]) sram_mem[address][15:8] <= wbs_dat_i[15:8];
            if (wbs_sel_i[2]) sram_mem[address][23:16] <= wbs_dat_i[23:16];
            if (wbs_sel_i[3]) sram_mem[address][31:24] <= wbs_dat_i[31:24];
          end
        end
        wbs_ack_o <= 1'b1;
      end else begin
  ...
  // Assign spike axon 
  assign spike_axon_0_o = { >> {sram_0}};
  assign spike_axon_1_o = { >> {sram_1}};
  
//   assign spike_axon_0_o = {
//     sram_0[0], sram_0[1], sram_0[2], sram_0[3], sram_0[4], sram_0[5], sram_0[6], sram_0[7]
//   };
//   assign spike_axon_1_o = {
//     sram_1[0], sram_1[1], sram_1[2], sram_1[3], sram_1[4], sram_1[5], sram_1[6], sram_1[7]
//   };
endmodule
